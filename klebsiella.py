...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  6 13:54:08 2020
...
#%%
new_table= cursor.execute("CREATE TABLE IF NOT EXISTS Klebsiella_ast_us AS SELECT * FROM Klebsiella WHERE AST_phenotypes LIKE '%=S%' OR AST_phenotypes LIKE '%=R%' OR AST_phenotypes LIKE '%=I%' ")
...
#%%
import pandas as pd
numrows= cursor.execute("SELECT AST_phenotypes FROM Klebsiella_ast_us WHERE collection_date LIKE '%2015%'  ")
...
df_ASTT.to_sql(name='klebsiella_astt_us', con=cnx, if_exists = 'replace', index=False)
...
numrows= cursor.execute("SELECT * FROM klebsiella_astt_us LEFT JOIN klebsiella_ast_us ON klebsiella_astt_us.AST_phenotypes=klebsiella_ast_us.AST_phenotypes  UNION SELECT * FROM klebsiella_astt_us RIGHT JOIN klebsiella_ast_us ON klebsiella_astt_us.AST_phenotypes=klebsiella_ast_us.AST_phenotypes")
print("Selected %s rows" %numrows)
print("Selected %s rows " %cursor.rowcount)
rows =cursor.fetchall()#fetch all rows at once
#print(tabulate(rows, headers=['strain','AST_phenotype', 'AMR_genotype'], tablefmt='psql'))    
...
##Cross-correlation matrix for Antimicrobial 
##%%
#import numpy as np
#import sys
#cross_df= df_AB
...
#    county=0
#    try:
#        sqlx_den=("SELECT COUNT("+ x +") FROM Klebsiella_ast_us WHERE "+ x +" <> 'NULL'")
#        print ("the query is",sqlx_den)
#        numrowsx_den= cursor.execute("SELECT COUNT("+ x +") FROM Klebsiella_ast_us WHERE "+ x +" <> 'NULL'")
...
#            #print(rows)
#        sqlx_num=("SELECT COUNT("+ x +") FROM Klebsiella_ast_us WHERE "+ x +"='R' or "+ x +"='I'")
#        print ("the query is",sqlx_num)
#        numrowsx_num= cursor.execute("SELECT COUNT("+x+") FROM Klebsiella_ast_us WHERE "+x+"='R' or " + x + "='I'")
...
#            print("the element in j axis is", y)
#            #county= county+1
#            sqly_num=("SELECT COUNT(" + y + ") FROM Klebsiella_ast_us where " + y + "='I' or " + y + "='R'")
#            numrowsy_num= cursor.execute("SELECT COUNT(" + y + ") FROM Klebsiella_ast_us where " + y + "='I' or " + y + "='R'")
...
#            
#            
#            sqly_den=("SELECT COUNT("+ x +") FROM Klebsiella_ast_us WHERE <> 'NULL'")
#            numrowsy_den= cursor.execute("SELECT COUNT("+ y +") FROM Klebsiella_ast_us WHERE "+ y +"  <> 'NULL'")
...
#%%
import pandas as pd
numrows= cursor.execute("Describe Klebsiella_full_us")
print("Selected %s rows" %numrows)
print("Selected %s rows " %cursor.rowcount)
rows =cursor.fetchall()#fetch all rows at once
print(rows)
...
listz=list(dfz[0])
numrows_k= cursor.execute("Select * from Klebsiella_full_us")
...
#df_kleb.to_excel("Klebsiella_ast_usfull.xlsx")
